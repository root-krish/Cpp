...
int main(){
    ...
    //Display all
    printf("Process Brust Waiting_Time TurnAround_Time ");
    for(i=0;i<n;i++){
        printf("\n%7d %5d %7d %3d",process[i],brustTime[i],WaitingTime[i],TurnAroundTime[i]);
    }
    printf("\nTotal Waiting Time=%d",TotalWaitingTime);
    printf("\nAvrage Turnarund Time=%f",AverageTurnAroundTime);
    printf("\nAvrage Waiting Time=%f",AverageWaitingTime);
    return 0;
}